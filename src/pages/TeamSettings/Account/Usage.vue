    ...
    <v-card-text>
      <v-row v-if="noExtra">
        ...
        <v-col cols="12" sm="5"
          ><div :class="smallScreen">
            You have used {{ usedTaskRuns }} task runs!</div
          >
          <div>
      ...
      <v-row v-else>
        <v-col cols="7">
          <ChartCard
            :chart-data="[
              { label: 'daysPassed', value: daysFromInvoice, color: '#ff8cc6' },
            ...
            accent-color="#fff"
            chart-overlay-center-type="text"
            :chart-overlay-center="`${usedTaskRuns}`"
            chart-overlay-center-two="Task Runs Used"
        /></v-col>
        <v-col cols="5"
          ><div>
            So far this month you have used {{ usedTaskRuns }} task runs.</div
          >
          <div> This will cost ${{ costSoFar }}. </div>
          <div>Your billing period resets on {{ subscriptionPeriodEnd }}.</div>
        </v-col>
      </v-row>
    </v-card-text>
  </v-card>
