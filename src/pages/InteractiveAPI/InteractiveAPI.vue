...
export default {
  ...
  methods: {
    ...
    openAutoComplete(editor, event) {
      if (event.code == 'Space' && event.ctrlKey) {
        this.open = true
        editor.showHint()
        this.autoCompletePosition = this.editor.cursorCoords()
        this.open = false
      }
    },
    ...
    configureEditor() {
      ...
      this.editor.on('keydown', this.openAutoComplete)
    ...
    addQuery(input) {
      const fields =
        input?.type?._fields ||
        input?.type?.ofType?._fields ||
        input?.type?.ofType?.ofType?._fields ||
        input?.type?.ofType?.ofType?.ofType?._fields
      const subSet = Object.entries(fields)
      const fieldKeys = subSet.map(entry => {
        const args = entry[1]?.args?.map(obj => `${obj.name} : ...`)
        const subArgs =
          this.includeArgs && args[0] ? `(${Object.values(args)})` : ''
        const subFields =
          entry[1]?.type?._fields ||
          entry[1]?.type?.ofType?._fields ||
          entry[1]?.type?.ofType?.ofType?._fields ||
          entry[1]?.type?.ofType?.ofType?.ofType?._fields
        return subFields
          ? `${entry[0]}` + subArgs + '{...}'
          : entry[0] + subArgs
      })
      const args = input.args?.map(obj => `${obj.name} : ...`)
      const argKeys = this.includeArgs ? `(${Object.values(args)})` : ''
      this.existing = this.editor.getValue()
      this.query = this.checkIf(input.action)
        ? `${this.existing} ${input.name}` + argKeys + `{ ${fieldKeys} }}`
        : `${input.action}  {${input.name}` +
          argKeys +
          `{ ${fieldKeys} }} ${this.existing}`

      this.editor.setValue(this.format(this.query))
    },
    checkIf(type) {
      const textArray = this.existing.split(' ')
      if (textArray[0] === type) {
        const last = this.existing.split('')
        last.pop()
        this.existing = last.join('')
        return true
      }
    },
...
<template>
  <v-container v-if="role === 'READ_ONLY_USER'" class="text-center">
    <div>
      <v-row>
        <v-col cols="2" />
        <v-col cols="8">
