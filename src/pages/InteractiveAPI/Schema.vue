...
export default {
  ...
  props: {
    ...
    includeSubs: { type: Boolean, default: false }
  },
  data() {
    return {
      typeColors: {
  ...
  methods: {
    ...
    buttonColor(type) {
      if (type === 'query') return '#0073df'
      return '#da2072'
    },
    includeArgs() {
      this.$emit('include-arg', this.incArgs)
    },
    addQuery(query) {
        ...
        <div v-for="item in schema" :key="item.name">
          ...
          <v-card-actions>
            ...
            <v-simple-checkbox
              ...
              @input="includeArgs"
            />
            <div label-for="include-input" class="subtitle-1 font-weight-light"
              >Include Arguments</div
            >
          </v-card-actions>
